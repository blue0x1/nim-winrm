...
<env:Body>
  <rsp:CommandLine CommandId="{cmdId}">
...
proc connectWinrmSocket(c: WinRMClient): Socket =
  ...
  result.connect(c.host, Port(c.port), timeout = 5000)
...
proc ntlmHandshake(c: var WinRMClient, body: string): RawHttpResponse =
  ...
  let r1 = rawNtlmPost(c, initialBody, "Negotiate " & encode(spnegoInit(neg)), c.ntlmSock)
  if getEnv("WINRMSHELL_DEBUG") == "1":
  ...
  if c.ntlmEncrypt:
    ...
    result = rawNtlmPost(c, encBody, "Negotiate " & encode(spnegoResp(auth)), c.ntlmSock, encCt)
    if getEnv("WINRMSHELL_DEBUG") == "1":
